...
vlc_gl_sub_renderer_Prepare(struct vlc_gl_sub_renderer *sr, subpicture_t *subpicture)
{
    ...
    if (subpicture) {
        ...
        for (subpicture_region_t *r = subpicture->p_region;
            ...
            //msg_Dbg(sr->gl, "r->i_x %d r->i_y %d glr->width %d glr->height %d subpicture->i_original_picture_width %d",
            //r->i_x, r->i_y, glr->width, glr->height, subpicture->i_original_picture_width);
            glr->texture = 0;
