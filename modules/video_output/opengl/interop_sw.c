...
#define LOAD_SYMBOL(type, name) \
    ...
    priv->has_unpack_subimage = vlc_gl_HasExtension(&extension_vt, "GL_EXT_unpack_subimage");
    ...
    priv->has_texture_rg = vlc_gl_HasExtension(&extension_vt, "GL_EXT_texture_rg");
...
static int OpenInteropSW(vlc_object_t *obj)
{
    struct vlc_gl_interop *interop = (void *) obj;
     msg_Dbg(interop->gl, "OpenInteropSW");
...
vlc_module_begin ()
    ...
    add_shortcut("sw")
    set_callbacks(OpenInteropSW, NULL)

    add_submodule()
    set_callbacks(OpenInteropDirectRendering, NULL)
