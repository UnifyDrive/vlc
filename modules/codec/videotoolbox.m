...
static bool HXXXGetBestChroma(decoder_t *p_dec)
{
    ...
    }else if (i_chroma_format == 2 /* YUV 4:2:2 */){
        if (i_depth_luma == 8 && i_depth_chroma == 8)
            p_sys->i_cvpx_format = kCVPixelFormatType_422YpCbCr8BiPlanarVideoRange;
        else if (i_depth_luma == 10 && i_depth_chroma == 10)
        {
#if !TARGET_OS_IPHONE
            if (deviceSupportsHEVC()) /* 42210bit went with HEVC on macOS */
                p_sys->i_cvpx_format = kCVPixelFormatType_422YpCbCr10;
            else
#endif
           /* Force BGRA output (and let VT handle the tone mapping) since the
            * Apple openGL* implementation can't handle 16 bit textures. This
...
#pragma clang diagnostic ignored "-Wpartial-availability"
    ...
    msg_Warn(p_dec, "[%s:%s:%d]=zspace=: videotoolbox-hw-decoder-only %d", __FILE__ , __FUNCTION__, __LINE__, var_InheritBool(p_dec, "videotoolbox-hw-decoder-only"));
