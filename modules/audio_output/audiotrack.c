...
static bool
AudioTrack_Check( JNIEnv *env, audio_output_t *p_aout, unsigned int i_rate,
                int i_channel_config, int i_format, int i_size );
static bool  GetAndroidSupport(JNIEnv *env, audio_output_t *p_aout,int i_rate,
...
struct aout_sys_t {
    ...
    bool  isz9x;
    int   i_dts_profile;
    vlc_fourcc_t i_ori_format;                          /**< audio format fourcc */
...
bytes_to_frames( aout_sys_t *p_sys, size_t i_bytes )
{
    ...
    if(p_sys->dts.i_frame_size != 0 && p_sys->dts.i_frame_length != 0) {
        return i_bytes * p_sys->fmt.i_frame_length * p_sys->dts.i_frame_length / p_sys->dts.i_frame_size;
    }
...
frames_to_bytes( aout_sys_t *p_sys, uint64_t i_frames )
{
    ...
    if(p_sys->dts.i_frame_size != 0 && p_sys->dts.i_frame_length != 0) {
        return i_frames * p_sys->dts.i_frame_size / p_sys->dts.i_frame_length / p_sys->fmt.i_frame_length;
    }
...
AudioTrack_getPlaybackHeadPosition( JNIEnv *env, audio_output_t *p_aout )
{
    ...
    if(i_pos == 0 && p_sys->headpos.i_last > 0 && p_sys->headpos.i_last < UINT32_MAX)
    {
        msg_Warn(p_aout, "position maybe have zero gap!.........");
...
AudioTrack_GetTimestampPositionUs( JNIEnv *env, audio_output_t *p_aout )
{
    ...
    //msg_Warn( p_aout, "[%s:%s:%d]=zspace=: p_sys->timestamp.i_frame_us=%lld, p_sys->timestamp.i_frame_pos=%lld. ", __FILE__ , __FUNCTION__, __LINE__, p_sys->timestamp.i_frame_us, p_sys->timestamp.i_frame_pos);
...
#if 0
{
    ...
    if( i_audiotrack_us > 0 )
    {
        //msg_Warn( p_aout, "i_audiotrack_us = %lld, i_samples_written_us=%lld(us), i_samples_written=%lld, frame_size=%d", i_audiotrack_us, FRAMES_TO_US( p_sys->i_samples_written ), p_sys->i_samples_written, p_sys->fmt.i_bytes_per_frame );
        /* AudioTrack delay */
        ...
        {
            msg_Warn( p_aout, "timing screwed, reset positions. i_samples_written=%lld, count=%d", p_sys->i_samples_written, p_sys->i_screwed_count );
            AudioTrack_ResetPositions( env, p_aout );
        }
    }
                ...
                int i_channel_config, int i_format, int i_size )
{
    ...
    if (p_audiotrack)
    {
     //  JNI_CALL_VOID(p_audiotrack ,jfields.AudioTrack.stop);
        JNI_CALL_VOID(p_audiotrack, jfields.AudioTrack.release);
        (*env)->DeleteLocalRef( env, p_audiotrack );
        p_audiotrack = NULL;
    }
...
static int
StartPassthrough( JNIEnv *env, audio_output_t *p_aout, bool ac3)
{
    aout_sys_t *p_sys = p_aout->sys;
    int i_at_format;
    bool b_dtshd = false;
    ...
    if(ac3 == true){
      ...
      //    p_sys->fmt.i_frame_length = 1;
        ...
        if( jfields.AudioFormat.has_ENCODING_IEC61937 )
        {
            ...
            switch( p_sys->fmt.i_format )
            {
                ...
                case VLC_CODEC_MLP:
                    ...
                    * apparently */
                    p_sys->fmt.i_physical_channels =  AOUT_CHANS_7_1; //AOUT_CHANS_STEREO;
                    break;
                case VLC_CODEC_DTS:
                    p_sys->fmt.i_bytes_per_frame = 4;
                    p_sys->fmt.i_physical_channels = AOUT_CHANS_STEREO;
        ...
        else
        {
            ...
            else if (p_sys->fmt.i_format != VLC_CODEC_TRUEHD && !b_dtshd ) {
                p_sys->fmt.i_bytes_per_frame = 4;
                p_sys->fmt.i_frame_length = 1;
                p_sys->fmt.i_physical_channels = AOUT_CHANS_STEREO;
                p_sys->fmt.i_channels = 2;
...
Start( audio_output_t *p_aout, audio_sample_format_t *restrict p_fmt )
{
    ...
    if( AOUT_FMT_LINEAR( &p_sys->fmt ) )
    {
        msg_Warn(p_aout, "[%s:%s:%d]=zspace=: StartPCM ....",__FILE__ , __FUNCTION__, __LINE__);
        i_ret = StartPCM( env, p_aout, i_max_channels );
    }else if( b_try_passthrough ){
        vlc_fourcc_t i_format_pass = p_sys->fmt.i_format;
        msg_Warn(p_aout, "[%s:%s:%d]=zspace=: StartPassthrough ....",__FILE__ , __FUNCTION__, __LINE__);
        ...
        vlc_fourcc_t i_format_pass = p_sys->fmt.i_format;
        msg_Warn(p_aout, "[%s:%s:%d]=zspace=: StartPassthrough eac3 ....",__FILE__ , __FUNCTION__, __LINE__);
...
ConvertFromIEC61937( audio_output_t *p_aout, block_t *p_buffer )
{
    ...
    //msg_Dbg( p_aout, "[%s:%s:%d]=zspace=: IEC61937 type [%x] size[%d] .", __FILE__ , __FUNCTION__, __LINE__, GetWBE( &p_buffer->p_buffer[4] ) & 0xFF, p_buffer->i_buffer);
...
Play( audio_output_t *p_aout, block_t *p_buffer )
{
    ...
    if( p_sys->b_passthrough && p_sys->fmt.i_format == VLC_CODEC_SPDIFB && (p_sys->i_ori_format == VLC_CODEC_EAC3 || p_sys->i_ori_format == VLC_CODEC_A52) && p_sys->a52.i_samples == 0 && p_sys->a52.i_size == 0)
    {
    ...
    if( p_sys->b_passthrough && p_sys->fmt.i_format == VLC_CODEC_SPDIFB && p_sys->i_ori_format == VLC_CODEC_DTS && p_sys->dts.i_frame_size == 0 && p_sys->dts.i_frame_length == 0)
    {
        vlc_dts_header_Parse(&(p_sys->dts), p_buffer->p_buffer, p_buffer->i_buffer);
        p_sys->i_max_audiotrack_samples = BYTES_TO_FRAMES( p_sys->audiotrack_args.i_size );
        msg_Dbg( p_aout, "[%s:%s:%d]=zspace=: dts.i_frame_size(%d), dts.i_frame_length(%d) .", __FILE__ , __FUNCTION__, __LINE__, p_sys->dts.i_frame_size, p_sys->dts.i_frame_length);
    }
...
static int DeviceSelect(audio_output_t *p_aout, const char *p_id)
{
    ...
    if( at_dev != p_sys->at_dev || i_encoding_flags != p_sys->i_encoding_flags )
    {
        ...
        if( at_dev == AT_DEV_ENCODED )
        {
            ...
            {
                bool b_dtshd;
                if( AudioTrack_HasEncoding( p_aout, enc_fourccs[i], &b_dtshd, false) )
