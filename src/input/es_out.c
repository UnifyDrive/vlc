...
static void EsOutProgramsChangeRate( es_out_t *out )
{
    ...
    for( int i = 0; i < p_sys->i_pgrm; i++ )
        input_clock_ChangeRate( VLC_OBJECT(p_sys->p_input),p_sys->pgrm[i]->p_clock, p_sys->i_rate );
}
...
static es_out_pgrm_t *EsOutProgramAdd( es_out_t *out, int i_group )
{
    ...
    i_max_caching = INT64_C(1000) * i_max_caching;

    input_clock_SetMaxCaching( p_pgrm->p_clock, i_max_caching);
